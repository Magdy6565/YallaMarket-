...
document.addEventListener("DOMContentLoaded", function() {
  ...
  // Show loading state
  orderDetailsContainer.innerHTML = '<div class="loading"><i class="fas fa-spinner fa-spin"></i> Loading order details...</div>';
  ...
  async function fetchOrderDetails(orderId) {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to fetch order details: Error ${response.status}`);
      }
  ...
  function renderOrderDetails(order) {
    if (!order) {
      orderDetailsContainer.innerHTML = '<div class="empty-state">No order data found.</div>';
      return;
    }
    ...
    const statusClass = order.status ? order.status.toLowerCase().replace(/\s/g, '-') : 'unknown';
      ...
      ? order.orderItems.map(item => `
        <div class="order-item">
          ...
          <div class="order-item-details">
            ...
            <p><strong>Price:</strong> $${item.priceEach ? item.priceEach.toFixed(2) : 'N/A'}</p>
            <p><strong>Subtotal:</strong> $${item.quantity && item.priceEach ? (item.quantity * item.priceEach).toFixed(2) : 'N/A'}</p>
            <button class="view-product-btn" data-product-id="${item.productId}">View Product Details</button>
          </div>
        </div>
      `).join('') 
    ...
    orderDetailsContainer.innerHTML = `
      <div class="order-details-card">
        ...
        <div class="order-metadata">
          ...
          <p><i class="fas fa-store"></i> Store ID: <strong>${order.storeId || 'N/A'}</strong></p>
        </div>
