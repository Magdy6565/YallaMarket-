  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401 || response.status === 403) {
        console.warn(
          "Authentication required or forbidden. Redirecting to login."
        );
    ...
    document.getElementById("profile-role").textContent = getRoleName(
      userData.role
    );
    document.getElementById("profile-email").textContent =
      userData.email || "N/A";
    document.getElementById("profile-contactInfo").textContent =
      userData.contactInfo || "N/A";
    ...
    if (userData.role === 1) {
      ...
      try {
        ...
        const accountSettingsSection = document.querySelector(
          ".profile-content .profile-section:last-child"
        );
        ...
        // This catch block now correctly closes the 'try' above it.
        console.error("Error loading vendor statistics:", error);
      }
    }
    // Statistics section is not shown for supermarket users
  } catch (error) {
    // This is the catch block for the outer `WorkspaceUserProfileAndDisplay` function.
    ...
    document.getElementById("profile-email").textContent =
      "Failed to load profile. Please ensure you are logged in and refresh.";
    document.getElementById("profile-contactInfo").textContent = "";
    document.getElementById("profile-address").textContent = "";
...
async function loadVendorStatistics(vendorId) {
  try {
    ...
    const totalProducts =
      backendData.totalProductsListed !== undefined
        ? backendData.totalProductsListed
        : 0;
    const totalOrders =
      backendData.totalOrdersReceived !== undefined
        ? backendData.totalOrdersReceived
        : 0;
    const totalRevenue =
      backendData.totalRevenue !== undefined ? backendData.totalRevenue : 0.0;
...
function updateStatsDisplay(
  ...
  try {
    ...
    const statsTotalProductsElement = document.getElementById(
      "stats-totalProducts"
    );
    ...
    const statsTotalOrdersElement =
      document.getElementById("stats-totalOrders");
    if (statsTotalOrdersElement) {
      statsTotalOrdersElement.textContent = totalOrders;
    }

    const statsTotalRevenueElement =
      document.getElementById("stats-totalRevenue");
    ...
    const statsOrdersLabelElement =
      document.getElementById("stats-ordersLabel");
    if (statsOrdersLabelElement) {
      statsOrdersLabelElement.textContent = isVendor
        ? "Orders Received"
        : "Orders Placed";
    }

    const statsRevenueLabelElement =
      document.getElementById("stats-revenueLabel");
    if (statsRevenueLabelElement) {
      statsRevenueLabelElement.textContent = isVendor
        ? "Total Revenue"
        : "Total Spent";
    }
  } catch (err) {
...
function showEditProfileModal() {
  ...
  const currentContactInfo = document.getElementById(
    "profile-contactInfo"
  ).textContent;
...
async function updateNotificationSettings(formData) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      throw new Error(
        errorData.message ||
          `Failed to update notification settings: Status ${response.status}`
      );
    }
  } catch (error) {
