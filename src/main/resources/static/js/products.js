...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function fetchAndDisplayProducts(category = '') {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                productsGridContainer.innerHTML = `<p style="color: red;">Error loading products: ${errorData.message || 'An unknown error occurred'}</p>`;
                return;
            }
    ...
    function renderProducts(products) {
        ...
        products.forEach(product => {
            ...
            const quantityClass = product.quantity > 10 ? 'in-stock' : (product.quantity > 0 ? 'low-stock' : 'out-of-stock');
            const stockIcon = product.quantity > 10 ? '<i class="fas fa-check-circle"></i>' :
                              (product.quantity > 0 ? '<i class="fas fa-exclamation-circle"></i>' :
                              '<i class="fas fa-times-circle"></i>');
            const stockText = product.quantity > 10 ? 'In Stock' :
                              (product.quantity > 0 ? `Low Stock: ${product.quantity} left` : 'Out of Stock');

            productCard.innerHTML = `
                <div class="product-image">
                    <img src="${product.imageUrl || 'https://via.placeholder.com/250'}" alt="${product.name || 'Product Image'}">
                </div>
                <div class="product-info">
                    ...
                    <div class="product-actions">
                        <a href="/products/${product.productId}" class="view-btn">
                            <i class="fas fa-eye"></i> View
                        </a>
                        <a href="/edit-product/${product.productId}" class="edit-btn">
                            <i class="fas fa-edit"></i> Edit
                        </a>
                        <button onclick="confirmDelete(${product.productId})" class="btn btn-sm btn-delete">
    ...
    async function populateCategoriesDropdown() {
        try {
            ...
            categories.forEach(category => {
                if (category) { // Ensure category string is not null or empty
...
function confirmDelete(productId) {
    ...
    // as alert/confirm are blocked in some environments or not user-friendly.
    if (confirm('Are you sure you want to delete this product? This action cannot be undone.')) {
        ...
        fetch(`/api/my-products/${productId}`, { // Adjust endpoint if needed
            method: 'DELETE',
            headers: headers
        })
        .then(response => {
            if (response.ok) {
                ...
                response.json().then(errorData => {
                    alert(`Failed to delete product: ${errorData.message || 'An unknown error occurred'}`);
                }).catch(() => {
